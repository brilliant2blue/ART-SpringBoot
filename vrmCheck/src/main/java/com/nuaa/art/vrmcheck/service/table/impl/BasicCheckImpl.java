...
import com.nuaa.art.vrmcheck.service.table.BasicCheck;
 ...
 */
@Service
public class BasicCheckImpl<T extends VariableRelationModel<?, Type, ConceptLibrary, ConceptLibrary, TableOfVRM, ModeClassOfVRM>> implements BasicCheck<T> {
     ...
     */
    @Override
    public boolean checkTypeBasic(T vrmModel, CheckErrorReporter errorReporter) {
     ...
     */
    @Override
    public boolean checkConstantBasic(T vrmModel, CheckErrorReporter errorReporter) {
     ...
     */
    @Override
    public boolean checkInputBasic(T vrmModel, CheckErrorReporter errorReporter) {
        if (!vrmModel.getInputs().isEmpty())
            variableBasicCheck(vrmModel, vrmModel.getInputs(), 1,errorReporter);
        return errorReporter.isBasicRight();
    }
     ...
     */
    @Override
    public boolean checkVariableBasic(T vrmModel, CheckErrorReporter errorReporter) {
        if (!vrmModel.getTerms().isEmpty()){
            variableBasicCheck(vrmModel, vrmModel.getTerms(), 2, errorReporter);
        }
        if (!vrmModel.getOutputs().isEmpty()){
            variableBasicCheck(vrmModel, vrmModel.getOutputs(), 3, errorReporter);
        }
        return errorReporter.isBasicRight();
    }
     ...
     */
    @Override
    public boolean checkModeClassBasic(T vrmModel, CheckErrorReporter errorReporter) {
     ...
     */
    @Override
    public boolean checkTableBasic(T vrmModel, CheckErrorReporter errorReporter) {
    ...
    private <VT extends ConceptLibrary> void  variableBasicCheck(T vrmModel, ArrayList<VT> Variables, int typeId, CheckErrorReporter errorReporter) {
        String varType = switch (typeId) {
            case 1 -> "输入变量";
            case 2 -> "中间变量";
            case 3 -> "输出变量";
            default -> "";
        };
        for (ConceptLibrary v : Variables) {
            String[] wrongLocationName = {"数据类型", "值域", "初始值"};
            boolean[] wrongLocation = {true, true, true, true};
            if (v.getConceptDatatype() == null || v.getConceptDatatype().isBlank()) {
                wrongLocation[1] = false;
                wrongLocation[0] = false;
            }
            if (v.getConceptRange() == null || v.getConceptRange().isBlank()) {
                wrongLocation[2] = false;
                wrongLocation[0] = false;
            }
            if (v.getConceptValue() == null || v.getConceptValue().isBlank()) {
            ...
            if (!wrongLocation[0]) {
                ...
                String outputString = "错误定位：" + varType + v.getConceptName() + "\n错误内容：该变量的";
                for (int i = 1; i < 4; i++) {
                    if (!wrongLocation[i])
                ...
                outputString = outputString.substring(0, outputString.length() - 1);
                outputString += "定义为空";
                var ci = new CheckErrorInfo(errorReporter.getErrorCount(), CheckErrorType.BasicVariableMiss, v.getConceptName(), outputString);
                ci.setRelateType(varType);
                errorReporter.addErrorList(ci);

            } else {
                String dataType = v.getConceptDatatype();
                Type thisType = null;
                for (Type type : vrmModel.getTypes()) {
                    if (type.getTypeName().equals(dataType)) {
                        thisType = type;
                ...
                if (thisType == null) {
                    BasicErrorRefresh(errorReporter);
                    String outputString = "错误定位：" + varType + v.getConceptName() + "\n错误内容：该变量的数据类型"
                            + v.getConceptDatatype() + "不存在";
                    var ci = new CheckErrorInfo(errorReporter.getErrorCount(), CheckErrorType.BasicVariableType, v.getConceptName(), outputString);
                    ci.setRelateType(varType);
                    errorReporter.addErrorList(ci);
                } else {
                    String value = v.getConceptValue();
                    boolean isValueViolateType = isValueNotComplyWithType(thisType, value);
                    if (isValueViolateType) {
                        BasicErrorRefresh(errorReporter);

                        String outputString = "错误定位："+ varType + v.getConceptName() + "\n错误内容：该变量的初始值不符合数据类型"
                                + v.getConceptDatatype();
                        var ci = new CheckErrorInfo(errorReporter.getErrorCount(), CheckErrorType.BasicVariableInit, v.getConceptName(), outputString);
                        ci.setRelateType(varType);
                        errorReporter.addErrorList(ci);
                    }
                }

                if (typeId != 1 && !v.getConceptDependencyModeClass().isEmpty()) {
                    boolean unfound = true;

                    String thismc = v.getConceptDependencyModeClass();
                    for (var mc : vrmModel.getModeClass()) {
                        if(mc.getModeClass().getModeClassName().equals(thismc)){
                            unfound = false; break;
                        }
                    }
                    ...
                    if(unfound){
                        ...
                        String outputString = "错误定位："+ varType + v.getConceptName() + "\n错误内容：该变量的依赖模式不存在"
                                + v.getConceptDependencyModeClass();
                        var ci = new CheckErrorInfo(errorReporter.getErrorCount(), CheckErrorType.BasicVariableModeClassMiss, v.getConceptName(), outputString);
                        ci.setRelateType(varType);
                        errorReporter.addErrorList(ci);
                    }
     ...
     * @return 2 变量类型错误
     */
    int isVariableUndefinedOrViolateType(T vrmModel, String variable, String variableValue) {
