...
@Service
public class ConditionPraserImpl implements com.nuaa.art.vrmcheck.service.obj.ConditionPraser {
     ...
     * 用于分析条件
     *
     * @return {@link ConditionsInformation}
     */
    @Override
    public ConditionsInformation emptyInformationFactory(){
     ...
     */
    @Override
    public void praserInformationInCondtions(VariableRealationModel vrmModel, List<TableRow> tableRows, ConditionsInformation c){
        c.nuclearTreeForEachRow = new ArrayList<>();
        c.continualVariables = new ArrayList<>();
        c.discreteVariables = new ArrayList<>();
        c.continualValues = new ArrayList<>();
        c.discreteRanges = new ArrayList<>();
        c.continualRanges = new ArrayList<>();
        ...
        sortContinualValues(c.continualValues);
        // 基于关键值的值域离散化
        c.variableRanges = rangeDiscretization(c.continualVariables.size(), c.discreteVariables.size(), c.continualValues, c.discreteRanges);
     ...
     * @param condition 条件
     */
    public void findCriticalVariableAndKeyValues(VariableRealationModel vrmModel, ConditionsInformation c, ConditionItem condition){
        ...
        if(variable!= null){
            ...
            if(typeUtils.WhetherContinuousRange(thisType)){ //连续型值域
                if (c.continualVariables.contains(condition.getVar1())) {// 变量已存储
                    int index = c.continualVariables.indexOf(condition.getVar1());
                    if (!c.continualValues.get(index).contains(condition.getVar2())) {// 值未存储
                        ArrayList<String> thisContinualValues = c.continualValues.get(index);
                        thisContinualValues.add(condition.getVar2());
                        c.continualValues.set(index,thisContinualValues);
                    }
                } else {// 变量未存储
                    c.continualVariables.add(condition.getVar1());
                    ...
                    thisContinualValues.add(condition.getVar2());
                    c.continualRanges.add(cr);
                    c.continualValues.add(thisContinualValues);
                }
            } else { //离散型值域
                if (!c.discreteVariables.contains(condition.getVar1())) {// 变量未存储
                    c.discreteVariables.add(condition.getVar1());
                    ArrayList<String> thisDiscreteRanges = (ArrayList<String>) typeUtils.GetRangeList(thisType);
                    c.discreteRanges.add(thisDiscreteRanges);
                }
            }
        } else { // 是模式的情况
            ...
            mc = vrmModel.getModeClass().stream()
                    ...
                    .findFirst().orElse(mc);
            if(!c.discreteVariables.contains(condition.getVar1())){
