...
public class AndEventCheckImpl implements EventCheck {
    ...
    public boolean checkEventConsistency(VRM vrmModel, CheckErrorReporter errorReporter) {
        for (TableOfVRM event: vrmModel.getEvents()) {
            ...
            for (String sourceMode : modes) {// 对同模式的条件进行一致性完整性判断
                ...
                System.out.println("事件"+subTable.stream().map(TableRow::getDetails).toString());
                AndEventsInformation ei = eventParser.emptyEventsInformationFactory();
                eventParser.praserInformationInTables(vrmModel, subTable, ei);
    ...
    public ArrayList<EventConsistencyError> checkSubEventTableConsistency(AndEventsInformation ei) {
